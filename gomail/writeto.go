...
func (w *messageWriter) formatFilename(name string) string {
	return fmt.Sprintf("=?UTF-8?B?%s?=", base64.StdEncoding.EncodeToString([]byte(name)))
}
...
func (w *messageWriter) addFiles(files []*file, isAttachment bool) {
	for _, f := range files {
		if _, ok := f.Header["Content-Type"]; !ok {
			...
			f.setHeader("Content-Type", mediaType+`; name="`+w.formatFilename(f.Name)+`"`)
		}
		...
		if _, ok := f.Header["Content-Disposition"]; !ok {
			...
			f.setHeader("Content-Disposition", disp+`; filename="`+w.formatFilename(f.Name)+`"`)
		}
